  ...
  try {
    ...
    switch (type) {
      ...
      case "boost_activated":
        messageText = `Habari ${data.landlord_name || "Landlord"}, your boost for "${data.property_title || "property"}" is now active! Duration: ${data.duration || "Featured"}. Get ready for more inquiries!`;
        break;
      case "lead_unlocked":
        messageText = `Habari ${data.landlord_name || "Landlord"}, lead unlocked successfully for "${data.property_title || "your property"}". You can now view tenant contact details on your dashboard.`;
        break;
      default:
