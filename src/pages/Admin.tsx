/**
 * FUTURE COMPONENT EXTRACTION CANDIDATES (TASK 4 ARCHITECTURAL ROADMAP):
 * -----------------------------------------------------------------------------
 * 1. AdminStatsOverview: Stats metrics grid (Total Revenue, Active Listings, Pending Payments, Users)
 * 2. AdminPendingPaymentsTable: Verification table & M-Pesa manual approval drawer
 * 3. AdminListingsTable: Listing moderation table, search filters, and status toggles
 * 4. AdminUsersTable: User roles, suspend/activate controls, and lead credit adjustments
 * 5. AdminBoostsAndUnlocksTabs: Transaction logs for boosted listings and lead credit purchases
 * -----------------------------------------------------------------------------
 */
...
import { 
  ...
  Download, Bell, Clock, Trash, AlertTriangle, ShieldCheck, Search, HardDrive
} from "lucide-react";
...
export const AdminPanel: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"payments" | "listings" | "users" | "boosts" | "unlocks" | "drive">("payments");
    ...
    try {
      const [
        ...
        supabase.from('listing_payments')
          ...
          .eq('status', 'confirmed'),
        supabase.from('listing_boosts')
          .select('amount')
          .eq('status', 'confirmed'),
        supabase.from('lead_unlocks')
          .select('amount_paid')
          .eq('status', 'confirmed'),
      ]);

      const listingRev = payments?.reduce((sum, p) => sum + Number(p.amount_paid || p.amount || 0), 0) || 0;
      const boostRev = boosts?.reduce((sum, b) => sum + Number(b.amount || 0), 0) || 0;
      const leadRev = unlocks?.reduce((sum, u) => sum + Number(u.amount_paid || 0), 0) || 0;
      const totalRev = listingRev + boostRev + leadRev;
      ...
      console.error("Failed to load statistics via direct Supabase query:", err);
    }
  ...
  const handleGrantCredits = async (propertyId: string, credits: number, customAmount: number) => {
    ...
    try {
      ...
      // 3. Update landlord profile balance and log to credit_transactions audit table
      if (prop.landlord_id) {
        ...
        const currentLandlordCredits = Number(landlordProf?.lead_credits || 0);
        await supabase
          .from('profiles')
          .update({ lead_credits: currentLandlordCredits + credits })
          .eq('id', prop.landlord_id);
        ...
        await supabase
          ...
          .insert({
            landlord_id: prop.landlord_id,
            property_id: propertyId,
            unlock_id: unlockData?.id || null,
            amount_paid: customAmount,
            credits_added: credits,
            type: 'admin_grant',
            notes: `Admin granted ${credits} lead credits`
          });
      }
      ...
      <div className="max-w-[900px] mx-auto pt-8 px-4" id="admin-panel-page">
        ...
        {loading ? (
          ...
          <div id="admin-tab-content-container">
            ...
            {activeTab === "drive" && (
              <div className="animate-fade-in space-y-4" id="google-drive-tab">
                <GoogleDriveManager inquiries={[]} propertyTitle="Admin System Backup" />
              </div>
            )}
