 ...
 * -----------------------------------------------------------------------------
 * 1. PropertyBasicDetailsStep: Unit category selection, title, rent, deposit, county, estate inputs
 * 2. PropertyLocationMapPicker: Interactive map pin & address search box
 * 3. PropertyAmenitiesSelector: Categorized checkboxes (Water/Utilities, Security, Features)
 * 4. PropertyPhotoUploaderSection: Image dropzone with primary photo ordering and virtual tour link
 * 5. PropertyMpesaPaymentModal: STK Push trigger & manual paybill verification for listing activation fee
 * -----------------------------------------------------------------------------
 */
...
export const ListProperty: React.FC = () => {
  ...
  // Step 4 Completion -> Insert raw property first in draft status ('unpaid' or 'verified' if pay-per-lead)
  const handleSavePropertyDraft = async () => {
    const activeUserId = profile?.id || user?.id;
    if (!activeUserId) {
      alert("Please ensure you are signed in before saving your listing.");
      return;
    }
    setPaying(true);
    ...
    try {
      if (!propertyId) {
        const payload: Record<string, any> = {
          ...
          payment_status: paymentModel === "pay_per_lead" ? "verified" : "unpaid",
          payment_model: paymentModel === "pay_per_lead" ? "pay_per_inquiry" : "pay_once",
          listing_model: listingModel
        };

        if (profile?.role === "agent") {
          payload.agent_id = activeUserId;
        }
              ...
              <div className="border border-[#E2EAE6] rounded-xl p-4 bg-white space-y-2">
                ...
                {uploadedImages.length > 0 ? (
                  <div className="space-y-2 pt-1">
                    ...
                    <div
                      className="flex gap-2 overflow-x-auto pb-1 select-none scrollbar-none"
                      style={{
